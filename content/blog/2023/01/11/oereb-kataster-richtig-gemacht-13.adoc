...
Für die Hintergrundkarte verwendet ich den ÖREB-Situationsplan von https://geodienste.ch/services/av/info[geodienste.ch]. Leider fehlt ein schweizweiter WMTS dieser Hintergrundkarte. Und aus diesem Grund wirkt es im vorliegenden Fall nicht so &laquo;snappy&raquo;
...
Was man zusätzlich automatisch durch eine Maschine testen sollte, ist der _layerIndex_ der Map-Elemente. Wenn sämtliche Elemente, also auch die Hintergrundkarte, den gleichen Wert aufweisen, weiss man nicht in welcher Reihenfolge die Bilder geschichtet werden müssen. Und wohl müsste im Regelfall (oder immer) die Hintergrundkarte - nomen est omen - tatsächlich im Hintergrund sein, also den kleinsten Wert aufweisen.
